// GET request to fetch data, once data is retrieved success
// callback willsetup d3
...
function setup() {
  $.ajax({
    ...
    success: (returnData) => {
      ...
      let yDollarScale = d3.scaleLinear()
        .domain([22.8, 23.9])
        .range([height, 0])

      //Create Axis
      let xTimeAxis = d3.axisBottom(xTimeScale)
      let yDollarAxis = d3.axisLeft(yDollarScale)
      ...
      .on("mousemove", function(d) {
        ...
        if (index === null) {
          $("#bids").html("<span class='ask-bid-title'>Bids:</span>" + `<span class='ask-bid-result'>$${bids[bids.length - 1] / 10000}</span>`)
          $("#asks").html("<span class='ask-bid-title'>Asks:</span>" + `<span class='ask-bid-result'>$${asks[asks.length - 1] / 10000}</span>`)
        } else {
          $("#bids").html("<span class='ask-bid-title'>Bids:</span>" + `<span class='ask-bid-result'>$${bids[index] / 10000}</span>`)
          $("#asks").html("<span class='ask-bid-title'>Asks:</span>" + `<span class='ask-bid-result'>$${asks[index] / 10000}</span>`)
        }
      })
      ...
      .on("mouseover", function(d) {
        ...
        $("#price").html("<div class='trade-result-container'><span class='ask-bid-title'>Price:</span>" + " " +  "<span class='trade-results'>" + "$" + (d.price / 10000) + "</span></div>")
